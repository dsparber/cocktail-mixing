...
double SPHKernels::wPoly6(double r2) {
    return  cPoly6 * pow(m_h2 - r2, 3);
}

Eigen::Vector3d SPHKernels::gwPoly6(const Eigen::Vector3d& r) {
    return cgPoly6 * pow(m_h2-r.squaredNorm(), 2) * r;
}

double SPHKernels::lwPoly6(double r2) {
    return cgPoly6 * (m_h2-r2) * (3*m_h2 - 7*r2);
}

double SPHKernels::wSpiky(double r) {
    return cSpiky * pow(m_h - r, 3);
}

Eigen::Vector3d SPHKernels::gwSpiky(const Eigen::Vector3d& r) {
    return cgSpiky * pow(m_h - r.norm(), 2) * r;
}

double SPHKernels::lwVisc(double r) {
    return clVisc * (m_h - r);
}