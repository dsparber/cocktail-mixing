...
void SphSimulation::updateDensityandPressure() {
    ...
    for(Particle& particle : m_particles) {
        ...
        for(Particle* neighbor : neighborhood) {
                ...
                particle.m_density += neighbor->m_mass * kernels::Wpoly6(r2, m_h);
        }
...
void SphSimulation::updateForce() {
    for(Particle& particle : m_particles) {
        ...
        for(Particle* neighbor : neighborhood) {
            ...
            double viscosity_coeff = neighbor->m_mass * kernels::lapWViscosity(r.squaredNorm(), m_h) / neighbor->m_density;
            f_pressure += pressure_coeff * kernels::gradWspiky(r, m_h);
...
void SphSimulation::updateVelocityandPosition() {
    for(Particle& particle : m_particles) {
        ...
        std::cout << "Pos: " << particle.m_pos.transpose() << " " << nextPosition.transpose() << std::endl;
